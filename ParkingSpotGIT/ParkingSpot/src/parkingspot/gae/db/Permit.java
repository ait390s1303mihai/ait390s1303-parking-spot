 ...
 * - "permitID" a {@link String} with the id of the permit ("1") <br>
 * - "fuelEfficient" a {@link String(Temporary, need to change to boolean)} determining fuel efficiency<br>
 */
...
public class Permit {
    ...
    /**
     *  The property name for the <b>permitID</b> of the permit.
     */
    private static final String PERMIT_ID_PROPERTY = "permitID";
    
    /**
     * Return the permit ID of the permit. 
     * @param user The Entity storing the permit.
     * @return a String with the login ID. 
     */
    public static String getPermitID(Entity permit) {
            Object val=permit.getProperty(PERMIT_ID_PROPERTY);
            if (val==null) return "";
            return (String) val;
    }
    ...
    /**
     *  The boolean for the <b>FUEL EFFICIENCY</b> of the permit.
     */
    private static String FUEL_EFFICIENCY_PROPERTY = "false";
    //TODO Change from String to Boolean
     ...
     * @return a String with the login ID. 
     */
    public static String getFuelEfficiency(Entity permit) {
            Object val=permit.getProperty(FUEL_EFFICIENCY_PROPERTY);
            if (val==null) return "";
            return (String) val;
    }
     ...
     */
    public static Entity createPermit(String permitID) {
            Entity permit = null;
            DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();
            Transaction txn = datastore.beginTransaction();
            try {
            
            		permit = getPermitWithPermitID(permitID);
                    if (permit!=null) {
                            return null;
                    }
                    
                    permit = new Entity(ENTITY_KIND);
                    permit.setProperty(PERMIT_ID_PROPERTY, permitID);
     ...
     * @return A GAE {@link Entity} for the permit or <code>null</code> if none or error.
     */
    public static Entity getPermitWithPermitID(String permitID) {
            DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();
            return getPermitWithPermitID(datastore, permitID);
    }
     ...
     */
    public static Entity getPermitWithPermitID(DatastoreService datastore, String permitID) {
            Entity permit = null;
            try {
                    
                    Filter hasPermitID =
                                      new FilterPredicate(PERMIT_ID_PROPERTY,
     ...
     */
    public static boolean updatePermitCommand(String permitID, String name, String fuelEfficient) {
            Entity permit = null;
            try {
                    permit = getPermit(permitID);
                    permit.setProperty(NAME_PROPERTY, name);
                    permit.setProperty(FUEL_EFFICIENCY_PROPERTY, fuelEfficient);
                    DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();
                    datastore.put(permit);
            } catch (Exception e) {
                    return false;
    ...
    public static Entity getPermit(User user) {
            String permitID = null;
            Entity permit = null;
            if (permit==null && user.getUserId()!=null) {
            		permitID=user.getUserId();
                    permit=getPermitWithPermitID(permitID);
            }
            /* 														NYI
            if (permit==null && user.getEmail()!=null) {
            		email=user.getEmail();
                    permit=getPermitWithEmail(email);
            }

            if (permit==null && user.getFederatedIdentity()!=null) {
